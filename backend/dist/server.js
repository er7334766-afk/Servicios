...
const COOKIE_SAME_SITE = rawSameSite === 'none' ? 'none' : rawSameSite === 'strict' ? 'strict' : 'lax';
...
async function subirArchivoAzure(base64, fileName, contentType, containerName) {
...
app.post('/api/upload-chat', async (req, res) => {
    try {
        ...
        const url = await subirArchivoAzure(base64, fileName, contentType, AZURE_CHAT_CONTAINER);
...
app.post('/api/empleados', async (req, res) => {
    try {
        ...
        const nombre = String(nombre_E ?? '').trim();
        const correoNormalizado = String(correo ?? '')
            .trim()
            .toLowerCase();
        const telefono = String(celular ?? '').trim();
        const password = String(password_E ?? '');
        if (!nombre ||
            !password ||
            !correoNormalizado ||
            !telefono) {
        ...
        const [resultadoCorreo,] = await database.execute(`
        SELECT TOP 1
          id_empleado
        FROM empleados
        WHERE LOWER(
        ...
        `, [correoNormalizado]);
        const empleadosExistentes = obtenerFilas(resultadoCorreo);
        ...
        const [resultadoRegistro,] = await database.execute(`
        ...
        const empleadosInsertados = obtenerFilas(resultadoRegistro);
        const empleado = empleadosInsertados[0];
        const idEmpleado = Number(empleado?.id_empleado ?? 0);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
            console.error('No se pudo obtener el ID del empleado registrado:', resultadoRegistro);
            return res.status(500).json({
                mensaje: 'El empleado fue registrado, pero no se pudo iniciar la sesión',
            });
        }
...
app.get('/api/empleados', async (_req, res) => {
    try {
        ...
        ORDER BY nombre ASC;
        `);
        const empleados = obtenerFilas(resultado);
...
app.get('/api/empleados/disponibles', async (_req, res) => {
    try {
        const [respuesta,] = await database.execute(`
          SELECT
            id_empleado,
            nombre,
            correo,
          ...
          `);
        const empleados = obtenerFilas(respuesta);
        return res.status(200).json(empleados);
    }
    catch (error) {
...
app.get("/api/empleados/:id", async (req, res) => {
    try {
      ...
      `);
        const empleados = Array.isArray(respuesta?.[0])
            ? respuesta[0]
...
app.put("/api/empleados/:id", async (req, res) => {
    try {
        const idEmpleado = Number(req.params.id);
        const { nombre_E, correo, celular, titulo, dni, antecedente, direccion, sobre_mi, foto, } = req.body;
        if (!Number.isInteger(idEmpleado) || idEmpleado <= 0) {
            return res.status(400).json({
                mensaje: "ID de empleado inválido",
            });
...
app.post('/api/clientes', async (req, res) => {
    try {
        const { nombre_C, password_C, correo, celular, } = req.body;
        const nombre = String(nombre_C ?? '').trim();
        const correoNormalizado = String(correo ?? '')
            .trim()
            .toLowerCase();
        const telefono = String(celular ?? '').trim();
        const password = String(password_C ?? '');
        if (!nombre ||
            !password ||
            !correoNormalizado ||
            !telefono) {
        ...
        `, [correoNormalizado]);
        const clientesExistentes = obtenerFilas(resultadoCorreo);
        ...
        const clientesInsertados = obtenerFilas(resultadoRegistro);
        const cliente = clientesInsertados[0];
        const idCliente = Number(cliente?.id_cliente ?? 0);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
            console.error('No se pudo obtener el ID del cliente registrado:', resultadoRegistro);
            return res.status(500).json({
                mensaje: 'El cliente fue registrado, pero no se pudo iniciar la sesión',
            });
        }
        const sid = randomUUID();
        const usuarioSesion = {
...
app.get("/api/clientes/:id", async (req, res) => {
    try {
        ...
        WHERE id_cliente = ${idCliente}
                `);
        const clientes = Array.isArray(respuesta?.recordset)
            ? respuesta.recordset
...
app.put("/api/clientes/:id", async (req, res) => {
    try {
        const idCliente = Number(req.params.id);
        const { nombre_C, correo, celular, dni, direccion, foto, } = req.body;
        ...
        if (celularLimpio &&
            ...
            return res.status(400).json({
                mensaje: "El celular debe contener exactamente 8 números",
            });
        }
        if (dniLimpio &&
            !/^\d+$/.test(dniLimpio)) {
            return res.status(400).json({
                mensaje: "El DNI solo puede contener números",
            });
        }
        const escaparSql = (valor) => valor.replace(/'/g, "''");
        const respuesta = await database.query(`
      UPDATE clientes
      SET
    ...
    `);
        const resultado = respuesta?.recordset?.[0] ??
            respuesta?.recordsets?.[0]?.[0] ??
            respuesta?.[0]?.[0] ??
            null;
        const filasActualizadas = Number(resultado?.filasActualizadas ?? 0);
...
app.get("/api/clientes/:id", async (req, res) => {
    try {
        ...
        const [clientes] = await database.execute(`
      SELECT
        id_cliente,
        nombre AS nombre_C,
        correo,
...
app.get('/api/categorias/:id/empleados', async (req, res) => {
    try {
        ...
        const [resultado,] = await database.execute(`
          ...
          `, [idCategoria]);
        const empleados = obtenerFilas(resultado);
        console.log(`Categoría ${idCategoria}:`, empleados.length, 'trabajadores visibles encontrados');
        return res.status(200).json(empleados);
    }
    catch (error) {
...
app.post("/api/login", async (req, res) => {
    try {
      ...
      WHERE correo = ?
    `, [correoLimpio]);
            const filas = Array.isArray(respuesta?.recordset)
                ? respuesta.recordset
                : Array.isArray(respuesta?.recordsets?.[0])
                    ? respuesta.recordsets[0]
            ...
            if (filas.length > 0) {
                ...
                if (valid)
                    usuario = (({ password_hash, ...rest }) => rest)(row);
            }
        }
        else if (rol === 'worker') {
            const [respuesta] = await database.execute(`
    ...
    `, [correoLimpio]);
        ...
        if (!usuario) {
            ...
            const updated = { count: prev.count + 1 };
...
function obtenerFilas(resultado) {
    ...
    if (Array.isArray(resultado.recordsets?.[0])) {
...
app.post('/api/servicios', async (req, res) => {
    try {
        const { fk_cliente, fk_categoria, fk_evidencia, titulo, descripcion, direccion, presupuesto, fecha, hora_inicio, hora_fin, } = req.body;
        ...
        const descripcionLimpia = typeof descripcion === 'string'
            ? descripcion.trim()
            : '';
        const direccionLimpia = typeof direccion === 'string'
            ? direccion.trim()
            : '';
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0 ||
            ...
            !hora_fin) {
            return res.status(400).json({
                mensaje: 'Faltan datos obligatorios o existen datos inválidos',
            });
        }
        if (String(hora_fin) <= String(hora_inicio)) {
            return res.status(400).json({
                mensaje: 'La hora final debe ser posterior a la hora inicial',
            });
        }
        // Crear servicio.
        const [resultadoServicio] = await database.execute(`
        ...
        const filasServicio = obtenerFilas(resultadoServicio);
        const idServicio = Number(filasServicio[0]?.id_servicio);
        ...
        const [resultadoEmpleados] = await database.execute(`
        SELECT DISTINCT
          ec.id_empleado
        ...
        `, [idCategoria]);
        const empleados = obtenerFilas(resultadoEmpleados);
        ...
        for (const empleado of empleados) {
            ...
            const [resultadoNotificacion] = await database.execute(`
          ...
          `, [
            ...
            const filasNotificacion = obtenerFilas(resultadoNotificacion);
...
// ==========================================
app.post('/api/servicios/:idServicio/postular', async (req, res) => {
    try {
        const idServicio = Number(req.params.idServicio);
        const idEmpleado = Number(req.body?.fk_empleado);
        const tipoPostulacion = String(req.body?.tipo_postulacion ?? 'aceptar')
            .trim()
            .toLowerCase();
        if (!Number.isInteger(idServicio) ||
            idServicio <= 0) {
        ...
        const [resultadoServicio] = await database.execute(`
          ...
          WHERE s.id_servicio = ?
          `, [idServicio]);
        const servicios = obtenerFilas(resultadoServicio);
        ...
        const estadoServicio = String(servicio.estado ?? '')
            .trim()
            .toLowerCase();
        if (![
            'pendiente',
            ...
            return res.status(400).json({
                mensaje: 'Este servicio ya no acepta postulaciones',
            });
        }
        const idCliente = Number(servicio.id_cliente);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
            return res.status(400).json({
                mensaje: 'El servicio no tiene un cliente válido',
            });
        }
        // Obtener empleado.
        const [resultadoEmpleado] = await database.execute(`
          SELECT TOP 1
            id_empleado,
            nombre
          FROM empleados
          WHERE id_empleado = ?
          `, [idEmpleado]);
        const empleados = obtenerFilas(resultadoEmpleado);
        if (empleados.length === 0) {
            return res.status(404).json({
        ...
        const [resultadoExistente] = await database.execute(`
          SELECT TOP 1
            id_postulacion,
            estado,
            fecha
        ...
        const existentes = obtenerFilas(resultadoExistente);
        if (existentes.length > 0) {
            return res.status(409).json({
                mensaje: 'Ya te postulaste a este servicio',
                postulacion: existentes[0],
            });
        }
        // Crear postulación.
        const [resultadoPostulacion] = await database.execute(`
        ...
        const postulaciones = obtenerFilas(resultadoPostulacion);
        const postulacion = postulaciones[0] ?? {
            fk_servicio: idServicio,
            fk_empleado: idEmpleado,
            estado: 'Pendiente',
        };
        const nombreEmpleado = String(empleado.nombre ||
            'Un trabajador').trim();
        const tituloServicio = String(servicio.titulo ||
            'Solicitud de servicio').trim();
        const descripcionNotificacion = tipoPostulacion === 'negociar'
        ...
        const notificaciones = obtenerFilas(resultadoNotificacion);
        console.log('Postulación y notificación creadas:', {
            idServicio,
            idEmpleado,
            idCliente,
            idPostulacion: postulacion?.id_postulacion ??
                null,
            idNotificacion: notificaciones[0]
                ?.id_notificacion ?? null,
        });
        return res.status(201).json({
            mensaje: 'Postulación registrada correctamente',
            postulacion,
            notificacion: notificaciones[0] ?? null,
        });
    }
    catch (error) {
...
// ==========================================
app.get('/api/clientes/:idCliente/notificaciones', async (req, res) => {
    try {
        ...
        const [resultado] = await database.execute(`
          ...
          `, [idCliente]);
        const notificaciones = obtenerFilas(resultado);
        return res.status(200).json({
            notificaciones,
            total: notificaciones.length,
            no_leidas: notificaciones.filter((item) => !Boolean(item.leida)).length,
        });
    }
    catch (error) {
...
// ==========================================
app.get('/api/empleados/:idEmpleado/notificaciones', async (req, res) => {
    try {
        ...
        console.error('Error al obtener notificaciones del empleado:', error);
        return res.status(500).json({
            mensaje: 'Error al obtener las notificaciones',
            detalle: error?.message || String(error),
        });
...
// ==========================================
app.put('/api/notificaciones/:idNotificacion/leer', async (req, res) => {
    try {
        const idNotificacion = Number(req.params.idNotificacion);
        if (!Number.isInteger(idNotificacion) ||
            idNotificacion <= 0) {
            return res.status(400).json({
...
// ==========================================
app.put('/api/clientes/:idCliente/notificaciones/leer-todas', async (req, res) => {
    try {
        const idCliente = Number(req.params.idCliente);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
        ...
        console.error('Error al marcar notificaciones del cliente:', error);
        return res.status(500).json({
...
// ==========================================
app.put('/api/empleados/:idEmpleado/notificaciones/leer-todas', async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
    ...
    try {
      ...
      `);
        ...
        */
        ...
        else if (Array.isArray(respuesta?.recordset)) {
            // Formato directo de mssql
            filas = respuesta.recordset;
        }
        else if (Array.isArray(respuesta?.rows)) {
...
app.put("/api/servicios/:id", async (req, res) => {
    try {
        ...
        const { fk_cliente, fk_categoria, fk_evidencia, descripcion, direccion, presupuesto, fecha, hora_inicio, hora_fin, } = req.body;
        if (!fk_cliente ||
            !fk_categoria ||
            !descripcion ||
            !direccion ||
            ...
            !hora_fin) {
            return res.status(400).json({
                mensaje: "Faltan datos obligatorios para actualizar la solicitud",
            });
        }
        if (String(hora_fin) <= String(hora_inicio)) {
            return res.status(400).json({
...
app.get("/api/categorias", async (_req, res) => {
    try {
        const [categorias] = await database.query("SELECT id_categoria, nombre FROM categorias");
        res.json(categorias);
    }
    catch (error) {
...
// ==========================================
app.get("/api/empleados/:idEmpleado/subcategorias", async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
        ...
        `);
        let subcategorias = [];
        if (Array.isArray(respuesta?.recordset)) {
            subcategorias =
                respuesta.recordset;
        }
        else if (Array.isArray(respuesta?.recordsets?.[0])) {
            subcategorias =
                respuesta.recordsets[0];
        }
        else if (Array.isArray(respuesta?.[0])) {
            subcategorias =
                respuesta[0];
        }
        else if (Array.isArray(respuesta?.rows)) {
        ...
        console.error("Error al consultar subcategorías del empleado:", error);
        return res.status(500).json({
            mensaje: "Error al consultar las subcategorías del empleado",
            detalle: error?.message ||
                "Error desconocido",
        });
    }
...
// ==========================================
app.put("/api/empleados/:idEmpleado/categorias", async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        ...
        const subcategoriasRecibidas = Array.isArray(req.body?.subcategorias)
            ...
            : [];
        const categorias = categoriasRecibidas
            .map((valor) => Number(valor))
            .filter((id, indice, arreglo) => Number.isInteger(id) && id > 0 && arreglo.indexOf(id) === indice);
        const subcategorias = subcategoriasRecibidas
            .map((valor) => Number(valor))
            .filter((id, indice, arreglo) => Number.isInteger(id) && id > 0 && arreglo.indexOf(id) === indice);
        console.log("================================");
        ...
        const respuestaEmpleado = await database.query(`
        SELECT TOP 1 id_empleado
        FROM empleados
        WHERE id_empleado = ${idEmpleado}
        `);
        const empleados = Array.isArray(respuestaEmpleado?.[0])
            ? respuestaEmpleado[0]
        ...
        for (const idCategoria of categorias) {
            const respuestaCategoria = await database.query(`
          SELECT TOP 1 id_categoria
          FROM categorias
          WHERE id_categoria = ${idCategoria}
          `);
            const filasCategoria = Array.isArray(respuestaCategoria?.[0])
        ...
        for (const idSubcategoria of subcategorias) {
            const respuestaSubcategoria = await database.query(`
          SELECT TOP 1
            id_subcategoria,
            fk_categoria
          FROM subcategorias
          ...
          `);
            const filasSubcategoria = Array.isArray(respuestaSubcategoria?.[0])
            ...
            const idCategoriaDeSubcategoria = Number(filasSubcategoria[0].fk_categoria);
            if (!categorias.includes(idCategoriaDeSubcategoria)) {
                return res.status(400).json({
                    mensaje: `La subcategoría ${idSubcategoria} no pertenece a una categoría seleccionada`,
                });
            }
        }
...
app.get('/api/admin/export/services', async (_req, res) => {
    try {
      ...
      `);
        ...
        const rows = (servicios || []).map((r) => header.map((h) => {
            const v = r[h] ?? '';
...
app.get('/api/admin/export/clients', async (_req, res) => {
    try {
        const [clientes] = await database.query(`SELECT id_cliente, nombre AS nombre_C, correo, telefono AS celular, dni FROM clientes ORDER BY id_cliente ASC`);
        const header = ['id_cliente', 'nombre_C', 'correo', 'celular', 'dni'];
        const rows = (clientes || []).map((r) => header.map((h) => String(r[h] ?? '').replace(/\"/g, '""')).join(','));
        const csv = [header.join(','), ...rows].join('\n');
        res.setHeader('Content-Type', 'text/csv');
...
// ==========================================
app.get("/api/empleados/:idEmpleado/categorias", async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
        ...
        return res.status(500).json({
            mensaje: "Error al consultar las categorías del empleado",
            detalle: error?.message ||
                "Error desconocido",
        });
    }
...
app.delete("/api/empleados/:idEmpleado/categorias/:idCategoria", async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
...
app.patch('/api/workers/:id/disponibilidad', async (req, res) => {
    ...
    try {
        const valorDisponible = disponible === true ||
            disponible === 'true' ||
            disponible === 1 ||
            disponible === '1';
        ...
        const [resultado] = await database.execute("UPDATE empleados SET estado = ? WHERE id_empleado = ?", [nuevoEstado, idParaActualizar]);
        if (resultado.affectedRows === 0) {
...
app.post("/api/chat", async (req, res) => {
    try {
        const { fk_cliente, fk_empleado, remitente, mensaje, } = req.body;
        const idCliente = Number(fk_cliente);
        ...
        const respuestaConversacion = await database.query(`
        SELECT TOP 1
          id_conversacion
      ...
      `);
        let conversaciones = [];
        if (Array.isArray(respuestaConversacion?.recordset)) {
            conversaciones =
                respuestaConversacion.recordset;
        }
        else if (Array.isArray(respuestaConversacion
            ?.recordsets?.[0])) {
            conversaciones =
                respuestaConversacion.recordsets[0];
        }
        else if (Array.isArray(respuestaConversacion?.[0])) {
            conversaciones =
                respuestaConversacion[0];
        }
        else if (Array.isArray(respuestaConversacion?.rows)) {
            conversaciones =
                respuestaConversacion.rows;
        }
        let idConversacion = Number(conversaciones[0]?.id_conversacion);
        if (!Number.isInteger(idConversacion) ||
            idConversacion <= 0) {
            const respuestaNuevaConversacion = await database.query(`
        ...
        `);
            ...
            if (Array.isArray(respuestaNuevaConversacion
                ?.recordset)) {
                nuevaConversacion =
                    respuestaNuevaConversacion
                        .recordset;
            }
            else if (Array.isArray(respuestaNuevaConversacion
                ?.recordsets?.[0])) {
                nuevaConversacion =
                    respuestaNuevaConversacion
                        .recordsets[0];
            }
            else if (Array.isArray(respuestaNuevaConversacion?.[0])) {
                nuevaConversacion =
                    respuestaNuevaConversacion[0];
            }
            idConversacion = Number(nuevaConversacion[0]
                ?.id_conversacion);
        }
        if (!Number.isInteger(idConversacion) ||
            idConversacion <= 0) {
        ...
        const remitenteBD = remitente === "empleado"
            ? "EMPLEADO"
            : "CLIENTE";
        const respuestaMensaje = await database.execute(`
        ...
        let mensajesInsertados = [];
        if (Array.isArray(respuestaMensaje?.recordset)) {
            mensajesInsertados =
                respuestaMensaje.recordset;
        }
        else if (Array.isArray(respuestaMensaje
            ?.recordsets?.[0])) {
            mensajesInsertados =
                respuestaMensaje.recordsets[0];
        }
        else if (Array.isArray(respuestaMensaje?.[0])) {
            mensajesInsertados =
                respuestaMensaje[0];
        }
        else if (Array.isArray(respuestaMensaje?.rows)) {
            mensajesInsertados =
                respuestaMensaje.rows;
        }
        const mensajeInsertado = mensajesInsertados[0] ?? {};
        return res.status(201).json({
            mensaje: "Mensaje enviado correctamente",
            chat: {
                id_chat: Number(mensajeInsertado.id_mensaje),
                fk_cliente: idCliente,
                fk_empleado: idEmpleado,
                remitente,
                mensaje: textoMensaje,
                entregado: mensajeInsertado.entregado ??
                    false,
                leido: mensajeInsertado.leido ??
                    false,
                fecha: mensajeInsertado.fecha ??
...
// ==========================================
app.delete("/api/chat/conversaciones", async (req, res) => {
    try {
        const { rol, idUsuario, contactos, } = req.body;
        const usuarioId = Number(idUsuario);
        const idsContactos = Array.isArray(contactos)
            ? contactos
                .map((id) => Number(id))
                .filter((id, indice, arreglo) => Number.isInteger(id) &&
                id > 0 &&
        ...
        if (idsContactos.length === 0) {
            return res.status(400).json({
                mensaje: "Debes seleccionar al menos una conversación",
            });
        }
        const listaIds = idsContactos.join(",");
        const condicion = rol === "client"
            ? `
        ...
        const datos = resultado?.recordset?.[0] ??
            ...
            null;
        return res.status(200).json({
            mensaje: "Conversaciones eliminadas correctamente",
            conversacionesEliminadas: Number(datos?.conversacionesEliminadas ?? 0),
        });
    }
    catch (error) {
        console.error("Error al eliminar conversaciones:", error);
        return res.status(500).json({
            mensaje: "Error al eliminar las conversaciones",
            detalle: error?.message ?? String(error),
        });
    }
});
...
// ==========================================
app.get('/api/chat/cliente/:idCliente/empleado/:idEmpleado', async (req, res) => {
    try {
        const idCliente = Number(req.params.idCliente);
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idCliente) ||
        ...
        const idConversacion = Number(conversaciones[0].id_conversacion);
        const respuestaMensajes = await database.query(`
        SELECT
      ...
      `);
        let mensajes = [];
        if (Array.isArray(respuestaMensajes?.recordset)) {
            mensajes =
                respuestaMensajes.recordset;
        }
        else if (Array.isArray(respuestaMensajes
            ?.recordsets?.[0])) {
            mensajes =
                respuestaMensajes.recordsets[0];
        }
        else if (Array.isArray(respuestaMensajes?.[0])) {
            mensajes =
                respuestaMensajes[0];
        }
        else if (Array.isArray(respuestaMensajes?.rows)) {
            mensajes =
...
// ==========================================
app.get("/api/chat/empleado/:idEmpleado/conversaciones", async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
        ...
        `);
        const conversaciones = Array.isArray(respuesta?.recordset)
            ? respuesta.recordset
            : Array.isArray(respuesta?.recordsets?.[0])
                ? respuesta.recordsets[0]
                : Array.isArray(respuesta?.[0])
        ...
        console.error("Error al consultar conversaciones del empleado:", error);
        return res.status(500).json({
...
// ==========================================
app.get("/api/chat/cliente/:idCliente/conversaciones", async (req, res) => {
    try {
        const idCliente = Number(req.params.idCliente);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
        ...
        console.error("Error al consultar conversaciones del cliente:", error);
        return res.status(500).json({
            mensaje: "Error al consultar las conversaciones",
            detalle: error?.message || String(error),
        });
...
app.put('/api/chat/leidos', async (req, res) => {
    try {
        const { fk_cliente, fk_empleado, lector, } = req.body;
        const idCliente = Number(fk_cliente);
        const idEmpleado = Number(fk_empleado);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
        ...
        const idConversacion = Number(conversaciones[0].id_conversacion);
        const remitenteContrario = lector === 'empleado'
            ? 'CLIENTE'
            : 'EMPLEADO';
        const resultado = await database.query(`
        ...
        return res.status(200).json({
            ...
            actualizados: resultado?.rowsAffected?.[0] ?? 0,
        });
    }
    catch (error) {
...
// ==========================================
app.get('/api/empleados/:idEmpleado/postulaciones', async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
        ...
        `);
        let empleados = [];
        if (Array.isArray(respuestaEmpleado?.recordset)) {
            empleados =
                respuestaEmpleado.recordset;
        }
        else if (Array.isArray(respuestaEmpleado?.recordsets?.[0])) {
            empleados =
                respuestaEmpleado.recordsets[0];
        }
        else if (Array.isArray(respuestaEmpleado?.[0])) {
            empleados =
                respuestaEmpleado[0];
        }
        else if (Array.isArray(respuestaEmpleado?.rows)) {
            empleados =
                respuestaEmpleado.rows;
        }
        else if (Array.isArray(respuestaEmpleado)) {
        ...
        const respuestaPostulaciones = await database.query(`
          SELECT
            id_postulacion,
            fk_servicio,
            fk_empleado,
        ...
        `);
        let postulaciones = [];
        if (Array.isArray(respuestaPostulaciones?.recordset)) {
            postulaciones =
                respuestaPostulaciones.recordset;
        }
        else if (Array.isArray(respuestaPostulaciones?.recordsets?.[0])) {
            postulaciones =
                respuestaPostulaciones.recordsets[0];
        }
        else if (Array.isArray(respuestaPostulaciones?.[0])) {
            postulaciones =
                respuestaPostulaciones[0];
        }
        else if (Array.isArray(respuestaPostulaciones?.rows)) {
            postulaciones =
                respuestaPostulaciones.rows;
        }
        else if (Array.isArray(respuestaPostulaciones)) {
            postulaciones =
        ...
        return res.status(500).json({
            mensaje: 'Error al obtener las postulaciones del empleado',
            detalle: error?.message ||
                String(error),
        });
    }
...
// ==========================================
app.get('/api/servicios/:idServicio/postulaciones', async (req, res) => {
    try {
        const idServicio = Number(req.params.idServicio);
        if (!Number.isInteger(idServicio) ||
            idServicio <= 0) {
        ...
        // OBTENER EL SERVICIO
        // ======================================
        const respuestaServicio = await database.query(`
          SELECT TOP 1
            s.id_servicio,
        ...
        `);
        let servicios = [];
        if (Array.isArray(respuestaServicio?.recordset)) {
            servicios =
                respuestaServicio.recordset;
        }
        else if (Array.isArray(respuestaServicio?.recordsets?.[0])) {
            servicios =
                respuestaServicio.recordsets[0];
        }
        else if (Array.isArray(respuestaServicio?.[0])) {
            servicios =
                respuestaServicio[0];
        }
        else if (Array.isArray(respuestaServicio?.rows)) {
            servicios =
                respuestaServicio.rows;
        }
        else if (Array.isArray(respuestaServicio)) {
            servicios =
                respuestaServicio;
        }
        if (servicios.length === 0) {
        ...
        return res.status(500).json({
            mensaje: 'Error al obtener las postulaciones',
            detalle: error?.message ||
                String(error),
        });
    }
...
// ==========================================
app.put('/api/servicios/:idServicio/aceptar', async (req, res) => {
    try {
        const idServicio = Number(req.params.idServicio);
        const idEmpleado = Number(req.body?.fk_empleado);
        if (!Number.isInteger(idServicio) ||
        ...
        const respuesta = await database.query(`
          ...
          END CATCH;
        `);
        const resultado = respuesta?.recordset ??
            respuesta?.recordsets?.[respuesta.recordsets.length - 1] ??
            respuesta?.[0] ??
            [];
        console.log('Empleado aceptado y notificado:', {
            idServicio,
            idEmpleado,
            resultado,
        });
        ...
        if (mensajeError.includes('El servicio no existe')) {
            return res.status(404).json({
                mensaje: 'El servicio no existe',
            });
        }
        if (mensajeError.includes('Este servicio ya fue asignado')) {
            return res.status(409).json({
                mensaje: 'Este servicio ya fue asignado',
            });
        }
        if (mensajeError.includes('El empleado no tiene una postulación pendiente')) {
            return res.status(404).json({
                mensaje: 'El empleado no tiene una postulación pendiente',
            });
        }
        return res.status(500).json({
            mensaje: 'Error al seleccionar al empleado',
...
app.put('/api/postulaciones/:idPostulacion/rechazar', async (req, res) => {
    try {
        ...
        `);
        const filas = respuesta?.recordset ??
            respuesta?.recordsets?.[respuesta.recordsets.length - 1] ??
            respuesta?.[0] ??
            [];
        const actualizadas = Number(filas?.[0]?.actualizadas ?? 0);
        if (actualizadas === 0) {
            return res.status(404).json({
                mensaje: 'La postulación no existe o ya fue respondida',
            });
        }
        return res.status(200).json({
        ...
        return res.status(500).json({
            mensaje: 'Error al rechazar la postulación',
            detalle: error?.message ||
                String(error),
        });
    }
...
// A LA OTRA PERSONA 
// ==========================================
app.put('/api/servicios/:idServicio/estado', async (req, res) => {
    try {
        const idServicio = Number(req.params.idServicio);
        const estadoRecibido = String(req.body?.estado ?? '')
            .trim()
            .toLowerCase()
            .replace(/_/g, ' ');
        const responsable = String(req.body?.actualizado_por ??
            req.body?.cancelado_por ??
            '')
            .trim()
            .toLowerCase();
        const motivoCancelacion = String(req.body?.motivo_cancelacion ?? '').trim();
        if (!Number.isInteger(idServicio) ||
            idServicio <= 0) {
            return res.status(400).json({
                mensaje: 'ID de servicio inválido',
        ...
        const estadoNormalizado = mapaEstados[estadoRecibido];
        if (!estadoNormalizado) {
            return res.status(400).json({
                mensaje: 'El estado indicado no es válido',
                estado_recibido: req.body?.estado ?? null,
            });
        }
        if (responsable !== 'cliente' &&
            responsable !== 'empleado') {
            return res.status(400).json({
                mensaje: 'Debes indicar quién actualizó el servicio',
                detalle: 'Usa "cliente" o "empleado" en actualizado_por o cancelado_por',
            });
        }
        // ======================================
        ...
        const servicio = servicios[0];
        const idCliente = Number(servicio.id_cliente);
        const idEmpleado = Number(servicio.id_empleado);
        const tituloServicio = String(servicio.titulo ??
            'Servicio').trim();
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
        ...
        if (estadoNormalizado !== 'Pendiente' &&
            (!Number.isInteger(idEmpleado) ||
                idEmpleado <= 0)) {
            return res.status(400).json({
                mensaje: 'El servicio no tiene un trabajador asignado',
            });
        }
        // ======================================
        // ACTUALIZAR ESTADO CON OUTPUT
        // ======================================
        const [resultadoActualizacion,] = await database.execute(`
        UPDATE servicios
        SET estado = ?
        OUTPUT
          INSERTED.id_servicio,
        ...
        const actualizados = obtenerFilas(resultadoActualizacion);
        ...
        if (tituloNotificacion &&
            tipoNotificacion) {
            const idClienteDestino = responsable === 'empleado'
                ? idCliente
                : null;
            const idEmpleadoDestino = responsable === 'cliente'
                ? idEmpleado
                : null;
            const [resultadoExistente,] = await database.execute(`
            SELECT TOP 1
        ...
        console.log('Servicio actualizado y notificado:', {
            idServicio,
            estadoNormalizado,
            responsable,
            idCliente,
            idEmpleado,
            destinatario: responsable === 'empleado'
                ? {
                    tipo: 'cliente',
                    id: idCliente,
                }
        ...
        console.error('Error al actualizar estado y notificar:', error);
        return res.status(500).json({
            mensaje: 'No se pudo actualizar el estado del servicio',
            detalle: error?.message ||
                String(error),
            numero: error?.number ?? null,
            codigo: error?.code ?? null,
...
// ==========================================
app.get("/api/empleados/:idEmpleado/servicios", async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) || idEmpleado <= 0) {
            return res.status(400).json({
        ...
        console.error("Error al consultar agenda del empleado:", error);
        return res.status(500).json({
...
app.post('/api/resenas', async (req, res) => {
    try {
        const { id_reserva, id_empleado, calificacion_general, puntualidad, calidad, comunicacion, comentario, } = req.body;
        ...
        if (!Number.isInteger(idReserva) ||
            idReserva <= 0) {
            return res.status(400).json({
                mensaje: 'Falta un id_reserva válido',
            });
        }
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
            return res.status(400).json({
                mensaje: 'Falta un id_empleado válido',
            });
        }
        if (!Number.isInteger(calificacionGeneral) ||
            calificacionGeneral < 1 ||
            calificacionGeneral > 5) {
            return res.status(400).json({
                mensaje: 'La calificación general debe estar entre 1 y 5',
            });
        }
        const parseCalificacionOpcional = (valor) => {
            if (valor === undefined ||
                valor === null ||
                valor === '') {
                return null;
        ...
        const puntualidadValue = parseCalificacionOpcional(puntualidad);
        const calidadValue = parseCalificacionOpcional(calidad);
        const comunicacionValue = parseCalificacionOpcional(comunicacion);
        const comentarioValue = typeof comentario === 'string'
        ...
        const [resultadoReserva,] = await database.execute(`
          SELECT TOP 1
            r.id_reserva,
            r.id_empleado,
            r.id_servicio
          ...
          WHERE r.id_reserva = ?;
          `, [idReserva]);
        const reservas = obtenerFilas(resultadoReserva);
        ...
        if (Number(reserva.id_empleado) !==
            idEmpleado) {
            return res.status(400).json({
                mensaje: 'El empleado no corresponde a esta reserva',
            });
        }
        const [resultadoExistente,] = await database.execute(`
          SELECT TOP 1
          ...
          WHERE id_reserva = ?;
          `, [idReserva]);
        const resenasExistentes = obtenerFilas(resultadoExistente);
        if (resenasExistentes.length > 0) {
            return res.status(409).json({
                mensaje: 'Esta reserva ya tiene una reseña',
            });
        }
        const [resultadoInsercion,] = await database.execute(`
          INSERT INTO resenas
          (
            id_reserva,
            id_empleado,
        ...
        const resenasInsertadas = obtenerFilas(resultadoInsercion);
        const resenaCreada = resenasInsertadas[0] ??
            null;
        if (!resenaCreada) {
            return res.status(500).json({
                mensaje: 'La reseña fue procesada, pero no se pudo confirmar el registro',
            });
        }
        return res.status(201).json({
...
// ==========================================
app.put('/api/resenas/:idResena/respuesta', async (req, res) => {
    try {
        const idResena = Number(req.params.idResena);
        const respuestaTexto = String(req.body?.respuesta ?? '').trim();
        // ======================================
        // VALIDAR DATOS
        // ======================================
        if (!Number.isInteger(idResena) ||
            idResena <= 0) {
            return res.status(400).json({
        ...
        if (respuestaTexto.length < 3) {
            return res.status(400).json({
                mensaje: 'La respuesta debe tener al menos 3 caracteres',
            });
        }
        if (respuestaTexto.length > 500) {
            return res.status(400).json({
                mensaje: 'La respuesta no puede superar los 500 caracteres',
            });
        }
        // ======================================
        // OBTENER SESIÓN
        // ======================================
        const sid = String(req.cookies?.[COOKIE_NAME] ?? '').trim();
        if (!sid) {
            return res.status(401).json({
                mensaje: 'Debes iniciar sesión para responder',
            });
        }
        const sesion = sessions.get(sid);
        if (!sesion?.user) {
            return res.status(401).json({
        ...
        const correoSesion = String(usuarioSesion.correo ?? '')
            .trim()
            .toLowerCase();
        if (!correoSesion) {
            return res.status(401).json({
                mensaje: 'La sesión no contiene un correo válido',
            });
        }
        // ======================================
        // IDENTIFICAR EMPLEADO POR CORREO
        // ======================================
        const [resultadoEmpleadoSesion,] = await database.execute(`
          SELECT TOP 1
            id_empleado,
            nombre,
            correo
          ...
          `, [correoSesion]);
        const empleadosSesion = obtenerFilas(resultadoEmpleadoSesion);
        if (empleadosSesion.length === 0) {
            return res.status(403).json({
                mensaje: 'La sesión actual no pertenece a un trabajador registrado',
            });
        }
        const empleadoSesion = empleadosSesion[0];
        const idEmpleadoSesion = Number(empleadoSesion.id_empleado);
        if (!Number.isInteger(idEmpleadoSesion) ||
            idEmpleadoSesion <= 0) {
            return res.status(401).json({
                mensaje: 'No se pudo identificar al trabajador autenticado',
            });
        }
        // ======================================
        // CONSULTAR RESEÑA
        // ======================================
        const [resultadoResena,] = await database.execute(`
          SELECT TOP 1
            r.id_resena,
            r.id_reserva,
            r.id_empleado,
          ...
          WHERE r.id_resena = ?;
          `, [idResena]);
        const resenas = obtenerFilas(resultadoResena);
        if (resenas.length === 0) {
            return res.status(404).json({
                mensaje: 'La reseña no existe',
            });
        }
        const resena = resenas[0];
        const idEmpleadoResena = Number(resena.id_empleado);
        // ======================================
        // VALIDAR PROPIETARIO DE LA RESEÑA
        // ======================================
        if (!Number.isInteger(idEmpleadoResena) ||
            idEmpleadoResena <= 0) {
            return res.status(400).json({
                mensaje: 'La reseña no tiene un trabajador válido asociado',
            });
        }
        if (idEmpleadoResena !==
            idEmpleadoSesion) {
            console.error('IDs de trabajador no coinciden:', {
                idResena,
                correoSesion,
                idEmpleadoSesion,
                idEmpleadoResena,
                empleadoSesion: empleadoSesion.nombre,
                empleadoResena: resena.nombre_empleado,
            });
            return res.status(403).json({
                mensaje: `La reseña pertenece al trabajador ${idEmpleadoResena}, pero la sesión actual pertenece al trabajador ${idEmpleadoSesion}`,
                diagnostico: {
                    id_resena: idResena,
                    correo_sesion: correoSesion,
                    id_empleado_sesion: idEmpleadoSesion,
                    nombre_empleado_sesion: empleadoSesion.nombre,
                    id_empleado_resena: idEmpleadoResena,
                    nombre_empleado_resena: resena.nombre_empleado,
                },
            });
        }
        // ======================================
        ...
        // ======================================
        const respuestaExistente = String(resena.respuesta_evaluado ?? '').trim();
        ...
        const actualizadas = obtenerFilas(resultadoActualizacion);
        if (actualizadas.length === 0) {
            return res.status(409).json({
                mensaje: 'La reseña ya fue respondida o no pudo actualizarse',
            });
        }
        const respuestaGuardada = actualizadas[0];
        logSecurity('review_response_created', {
            idResena,
        ...
        return res.status(200).json({
            ...
            respuesta: {
                id_resena: Number(respuestaGuardada.id_resena),
                id_reserva: Number(respuestaGuardada.id_reserva),
                id_empleado: Number(respuestaGuardada.id_empleado),
                respuesta_evaluado: respuestaGuardada
                    .respuesta_evaluado,
                fecha_respuesta: respuestaGuardada
                    .fecha_respuesta,
            },
        });
    }
        ...
        return res.status(500).json({
            mensaje: 'Error al publicar la respuesta',
            detalle: error?.message ??
                String(error),
        });
    }
...
// ==========================================
app.get('/api/resenas/servicio/:idServicio', async (req, res) => {
    try {
        const idServicio = Number(req.params.idServicio);
        if (!Number.isInteger(idServicio) ||
            idServicio <= 0) {
        ...
        const [resultado,] = await database.execute(`
          ...
          ORDER BY
            ...
            r.id_resena DESC;
          `, [idServicio]);
        const resenas = obtenerFilas(resultado);
        if (resenas.length === 0) {
            return res.status(404).json({
                mensaje: 'Este servicio todavía no tiene una reseña',
                resena: null,
            });
        }
        return res.status(200).json({
        ...
        console.error('Error al consultar reseña del servicio:', error);
        return res.status(500).json({
            mensaje: 'Error al consultar la reseña del servicio',
            detalle: error?.message ??
                String(error),
        });
    }
});
app.put("/api/usuarios/actividad", async (req, res) => {
    try {
...
// ==========================================
app.get("/api/chat/contactos/:rol/:idUsuario", async (req, res) => {
    try {
        ...
        if (rol === "client") {
          ...
          ORDER BY nombre ASC
        `);
            const contactos = respuesta?.recordset ??
                respuesta?.recordsets?.[0] ??
                respuesta?.[0] ??
                respuesta?.rows ??
                [];
...
app.post('/api/payment-methods', async (req, res) => {
    try {
        ...
        const numeroEnmascarado = String(req.body?.numero_enmascarado ?? '').trim();
        const expiracion = String(req.body?.expiracion ?? '').trim();
        if (!Number.isInteger(fkUsuario) ||
            fkUsuario <= 0) {
            return res.status(400).json({
                mensaje: 'ID de usuario inválido',
            });
        ...
        if (!tipo ||
            ...
            !numeroEnmascarado) {
            return res.status(400).json({
                mensaje: 'Faltan datos del método de pago',
            });
        }
        if (!/\d{4}$/.test(numeroEnmascarado)) {
            return res.status(400).json({
                mensaje: 'El número enmascarado no es válido',
            });
        }
        if (expiracion &&
            !/^\d{2}\/\d{2}$/.test(expiracion)) {
            return res.status(400).json({
                mensaje: 'La expiración debe tener el formato MM/AA',
            });
        }
        const [resultado,] = await database.execute(`
        ...
        const metodosInsertados = obtenerFilas(resultado);
        const metodoGuardado = metodosInsertados[0] ??
            null;
        if (!metodoGuardado) {
            return res.status(500).json({
                mensaje: 'El método fue procesado, pero no se pudo confirmar el registro',
            });
        }
        return res.status(201).json({
            mensaje: 'Método de pago agregado correctamente',
            metodo: metodoGuardado,
        });
    }
    catch (error) {
...
app.get('/api/payment-methods/:fk_usuario', async (req, res) => {
    try {
        const fkUsuario = Number(req.params.fk_usuario);
        if (!Number.isInteger(fkUsuario) ||
            fkUsuario <= 0) {
            return res.status(400).json({
                mensaje: 'ID de usuario inválido',
        ...
        const metodos = obtenerFilas(resultado);
...
// ==========================================
app.get('/api/clientes/:idCliente/historial', async (req, res) => {
    try {
        const idCliente = Number(req.params.idCliente);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
        ...
        const respuesta = await database.query(`
          ...
          ORDER BY
            ...
            s.id_servicio DESC;
        `);
        const servicios = Array.isArray(respuesta?.recordset)
            ? respuesta.recordset
            : Array.isArray(respuesta?.recordsets?.[0])
                ? respuesta.recordsets[0]
                : Array.isArray(respuesta?.[0])
        ...
        const resenas = servicios
            .filter((servicio) => Number(servicio.tiene_resena) === 1)
            .map((servicio) => ({
            id: servicio.id_resena,
            bookingId: servicio.id_servicio,
            reviewerName: servicio.nombre_empleado ||
                'Trabajador',
            rating: Number(servicio.calificacion_general) || 0,
            comment: servicio.comentario_resena ||
                '',
            date: servicio.fecha_resena ||
                '',
        }));
        return res.status(200).json({
            servicios,
        ...
        console.error('Error al consultar historial del cliente:', error);
        return res.status(500).json({
            mensaje: 'Error al consultar el historial de contrataciones',
            detalle: error?.message ||
                String(error),
        });
    }
...
// ==========================================
app.get('/api/empleados/:idEmpleado/resumen-perfil', async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
        ...
        // ======================================
        const [resultadoResumen,] = await database.execute(`
          SELECT
            e.id_empleado,
          ...
          GROUP BY
            e.id_empleado;
          `, [idEmpleado]);
        const filasResumen = obtenerFilas(resultadoResumen);
        ...
        // ======================================
        const [resultadoResenas,] = await database.execute(`
          SELECT
            r.id_resena,
            r.id_reserva,
            r.calificacion_general,
          ...
          ORDER BY
            ...
            r.id_resena DESC;
          `, [idEmpleado]);
        const resenas = obtenerFilas(resultadoResenas);
        return res.status(200).json({
            total_trabajos: Number(resumen.total_trabajos) || 0,
            total_resenas: Number(resumen.total_resenas) || 0,
            promedio_calificacion: Number(resumen.promedio_calificacion) || 0,
            resenas,
        });
    }
    catch (error) {
        console.error('Error al consultar resumen del empleado:', error);
        return res.status(500).json({
            mensaje: 'Error al consultar el perfil del empleado',
            detalle: error?.message ??
                String(error),
        });
    }
...
// ==========================================
app.get('/api/clientes/:idCliente/perfil-publico', async (req, res) => {
    try {
        const idCliente = Number(req.params.idCliente);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
        ...
        // ======================================
        const resultadoCliente = await database.query(`
          SELECT TOP 1
            c.id_cliente,
          ...
          WHERE c.id_cliente =
            ${idCliente};
        `);
        const resultadoClienteReal = Array.isArray(resultadoCliente)
            ? resultadoCliente[0]
            : resultadoCliente;
        const clientes = obtenerFilas(resultadoClienteReal);
        if (clientes.length === 0) {
            return res.status(404).json({
        ...
        const resultadoServicios = await database.query(`
          ...
          ORDER BY
            ...
            s.id_servicio DESC;
        `);
        const resultadoServiciosReal = Array.isArray(resultadoServicios)
            ? resultadoServicios[0]
            : resultadoServicios;
        const ultimosServicios = obtenerFilas(resultadoServiciosReal);
        return res.status(200).json({
            ...clientes[0],
            ultimosServicios,
        });
        ...
        console.error('Error al obtener perfil público del cliente:', error);
        return res.status(500).json({
            mensaje: 'No se pudo cargar el perfil público del cliente',
            detalle: error?.message ||
                String(error),
        });
    }
...
// ==========================================
app.delete('/api/clientes/:idCliente/notificaciones', async (req, res) => {
    try {
        const idCliente = Number(req.params.idCliente);
        if (!Number.isInteger(idCliente) ||
            idCliente <= 0) {
        ...
        console.error('Error al eliminar notificaciones del cliente:', error);
        return res.status(500).json({
...
// ==========================================
app.delete('/api/empleados/:idEmpleado/notificaciones', async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
        ...
        console.error('Error al eliminar notificaciones del empleado:', error);
        return res.status(500).json({
            mensaje: 'No se pudieron eliminar las notificaciones',
            detalle: error?.message ||
                String(error),
...
//PRESUPUESTO 
/***************** */
app.put('/api/servicios/:idServicio/presupuesto', async (req, res) => {
    try {
        const idServicio = Number(req.params.idServicio);
        const presupuestoNuevo = Number(req.body?.presupuesto);
        if (!Number.isInteger(idServicio) ||
            idServicio <= 0) {
            return res.status(400).json({
                mensaje: 'ID de servicio inválido',
        ...
        if (!Number.isFinite(presupuestoNuevo) ||
            presupuestoNuevo <= 0) {
            return res.status(400).json({
        ...
        const servicio = servicios[0];
        const nombreCliente = String(servicio.nombre_cliente || 'El cliente').trim();
        const presupuestoAnterior = Number(servicio.presupuesto);
        const idEmpleado = Number(req.body?.fk_empleado ??
        ...
        const [resultadoEstado] = await database.execute(`
          UPDATE postulaciones
...
app.put('/api/postulaciones/:idPostulacion/aceptar-negociacion', async (req, res) => {
    try {
        const idPostulacion = Number(req.params.idPostulacion);
        if (!Number.isInteger(idPostulacion) ||
            idPostulacion <= 0) {
        ...
        const postulacion = postulaciones[0];
        const tipoPostulacion = String(postulacion.tipo_postulacion ?? '')
            .trim()
            .toLowerCase();
        const estadoNegociacion = String(postulacion.estado_negociacion ?? '')
            .trim()
            .toLowerCase();
        if (tipoPostulacion !== 'negociar' ||
            estadoNegociacion !==
                'esperandoconfirmacion') {
            return res.status(400).json({
                mensaje: 'Esta negociaci├│n no est├í pendiente de confirmaci├│n',
            });
        }
        await database.execute(`
        UPDATE postulaciones
        ...
        WHERE id_postulacion = ?
        `, [idPostulacion]);
        const idCliente = Number(postulacion.id_cliente);
        if (Number.isInteger(idCliente) &&
            idCliente > 0) {
            const nombreEmpleado = String(postulacion.nombre_empleado ||
                'El trabajador').trim();
            const tituloServicio = String(postulacion.titulo ||
                'Solicitud de servicio').trim();
            const presupuesto = Number(postulacion.presupuesto);
            await database.execute(`
          INSERT INTO notificaciones (
            id_cliente,
            id_empleado,
        ...
        return res.status(200).json({
            mensaje: 'Nuevo presupuesto aceptado correctamente',
            estado_negociacion: 'Aceptado',
        });
    }
    catch (error) {
        console.error('Error al aceptar negociación:', error);
        return res.status(500).json({
            mensaje: 'No se pudo aceptar el nuevo presupuesto',
            detalle: error?.message ?? String(error),
        });
    }
});
...
app.delete('/api/account', async (req, res) => {
    try {
        ...
        const sesionActual = sessions.get(sid);
        const user = sesionActual?.user;
        if (!user) {
            sessions.delete(sid);
            res.clearCookie(COOKIE_NAME);
            return res.status(401).json({
        ...
        const normalizarRol = (valor) => {
            ...
            if (rolesEmpleado.includes(rol)) {
                return 'worker';
            }
            if (rolesCliente.includes(rol)) {
                return 'client';
            }
            return '';
        };
        let role = normalizarRol(req.body?.role ??
            req.body?.rol ??
            req.body?.tipo ??
        ...
        const idEmpleadoSesion = Number(user.id_empleado ??
            user.idEmpleado ??
            0);
        const idClienteSesion = Number(user.id_cliente ??
            user.idCliente ??
            0);
        if (!role &&
            Number.isInteger(idEmpleadoSesion) &&
            idEmpleadoSesion > 0) {
            role = 'worker';
        }
        if (!role &&
            Number.isInteger(idClienteSesion) &&
            idClienteSesion > 0) {
            role = 'client';
        }
        const idGenerico = Number(user.id ?? 0);
        if (!role &&
            Number.isInteger(idGenerico) &&
            idGenerico > 0) {
            const [resultadoEmpleadoExiste,] = await database.execute(`
            SELECT TOP 1
              id_empleado
            FROM empleados
            WHERE id_empleado = ?;
            `, [idGenerico]);
            const empleadosEncontrados = obtenerFilas(resultadoEmpleadoExiste);
            const [resultadoClienteExiste,] = await database.execute(`
            SELECT TOP 1
              id_cliente
            FROM clientes
            WHERE id_cliente = ?;
            `, [idGenerico]);
            const clientesEncontrados = obtenerFilas(resultadoClienteExiste);
        ...
        const correoSesion = String(user.correo ??
            user.email ??
            '').trim();
        if (!role &&
            correoSesion) {
            const [resultadoEmpleadoCorreo,] = await database.execute(`
            SELECT TOP 1
              id_empleado
            FROM empleados
            WHERE LOWER(
            ...
            `, [correoSesion]);
            const empleadosCorreo = obtenerFilas(resultadoEmpleadoCorreo);
            const [resultadoClienteCorreo,] = await database.execute(`
            SELECT TOP 1
              id_cliente
            FROM clientes
            WHERE LOWER(
            ...
            `, [correoSesion]);
            const clientesCorreo = obtenerFilas(resultadoClienteCorreo);
            if (empleadosCorreo.length > 0 &&
                clientesCorreo.length === 0) {
                role = 'worker';
            }
            else if (clientesCorreo.length > 0 &&
        ...
        let userId = 0;
        if (role === 'worker') {
            userId = Number(user.id_empleado ??
                user.idEmpleado ??
                user.id ??
                0);
        }
        if (role === 'client') {
            userId = Number(user.id_cliente ??
                user.idCliente ??
                user.id ??
                0);
        }
        if (role === 'worker' &&
            (!Number.isInteger(userId) ||
                userId <= 0) &&
            correoSesion) {
            const [resultadoIdEmpleado,] = await database.execute(`
            SELECT TOP 1
              id_empleado
            FROM empleados
            WHERE LOWER(
            ...
            `, [correoSesion]);
            const empleados = obtenerFilas(resultadoIdEmpleado);
            userId = Number(empleados[0]?.id_empleado ??
                0);
        }
        if (role === 'client' &&
            (!Number.isInteger(userId) ||
                userId <= 0) &&
            correoSesion) {
            const [resultadoIdCliente,] = await database.execute(`
            SELECT TOP 1
              id_cliente
            FROM clientes
            WHERE LOWER(
            ...
            `, [correoSesion]);
            const clientes = obtenerFilas(resultadoIdCliente);
            userId = Number(clientes[0]?.id_cliente ??
                0);
        }
        if (role !== 'client' &&
            role !== 'worker') {
            console.error('No se pudo determinar el rol:', {
                body: req.body,
                query: req.query,
                usuarioSesion: user,
            });
            return res.status(400).json({
                ...
                detalle: 'No se pudo identificar si la cuenta es de cliente o trabajador',
            });
        }
        if (!Number.isInteger(userId) ||
            userId <= 0) {
            console.error('No se pudo determinar el ID:', {
                role,
                usuarioSesion: user,
            });
            return res.status(400).json({
        ...
        // ======================================
        const TIEMPO_ESPERA_MS = 5 * 60 * 1000;
        const tabla = role === 'worker'
            ? 'empleados'
            : 'clientes';
        const columnaId = role === 'worker'
            ? 'id_empleado'
            : 'id_cliente';
        const [resultadoFecha,] = await database.execute(`
          SELECT
            fecha_creacion
          FROM ${tabla}
          WHERE ${columnaId} = ?;
          `, [userId]);
        const filasFecha = obtenerFilas(resultadoFecha);
        if (filasFecha.length === 0) {
            return res.status(404).json({
                mensaje: role === 'worker'
        ...
        const fechaCreacionRaw = filasFecha[0]
            ?.fecha_creacion;
        const fechaCreacion = fechaCreacionRaw instanceof Date
            ? fechaCreacionRaw
            : new Date(fechaCreacionRaw);
        const fechaCreacionMs = fechaCreacion.getTime();
        if (!Number.isFinite(fechaCreacionMs)) {
            ...
            return res.status(500).json({
                mensaje: 'No se pudo validar la antigüedad de la cuenta',
                detalle: 'La fecha de creación almacenada no es válida',
            });
        }
        ...
        if (tiempoTranscurrido <
            TIEMPO_ESPERA_MS) {
            const segundosRestantes = Math.max(1, Math.ceil((TIEMPO_ESPERA_MS -
                tiempoTranscurrido) /
                1000));
            const minutosRestantes = Math.ceil(segundosRestantes /
                60);
            return res.status(429).json({
                mensaje: 'Debes esperar antes de eliminar la cuenta.',
                detalle: `Podrás eliminarla en aproximadamente ${minutosRestantes} minuto(s).`,
        ...
        // ======================================
        if (role === 'client') {
            const correoAnonimo = `eliminado_cliente_${userId}_${marcaTiempo}@cuenta.local`;
            const [resultadoCliente,] = await database.execute(`
            UPDATE clientes
            SET
              nombre =
                'Cuenta eliminada',
            ...
            const clientesActualizados = obtenerFilas(resultadoCliente);
            if (clientesActualizados.length ===
                0) {
                return res.status(404).json({
                    mensaje: 'La cuenta del cliente no existe',
                });
            }
        }
        // ======================================
        ...
        // ======================================
        if (role === 'worker') {
            const correoAnonimo = `eliminado_empleado_${userId}_${marcaTiempo}@cuenta.local`;
            const [resultadoEmpleado,] = await database.execute(`
            UPDATE empleados
            SET
              nombre =
            ...
            const empleadosActualizados = obtenerFilas(resultadoEmpleado);
            if (empleadosActualizados.length ===
                0) {
                return res.status(404).json({
                    mensaje: 'La cuenta del trabajador no existe',
                });
            }
        }
        // ======================================
        ...
        for (const [sessionId, sessionData,] of sessions.entries()) {
            const usuarioSesion = sessionData?.user;
            if (!usuarioSesion) {
                continue;
            }
            const idSesion = role === 'worker'
                ? Number(usuarioSesion.id_empleado ??
                    usuarioSesion.idEmpleado ??
                    usuarioSesion.id ??
                    0)
                : Number(usuarioSesion.id_cliente ??
                    usuarioSesion.idCliente ??
                    usuarioSesion.id ??
                    0);
            const correoOtraSesion = String(usuarioSesion.correo ??
                usuarioSesion.email ??
                '')
                .trim()
                .toLowerCase();
            const mismaCuentaPorId = idSesion === userId;
            const mismaCuentaPorCorreo = Boolean(correoSesion) &&
                correoOtraSesion ===
        ...
        return res.status(500).json({
            mensaje: 'Error interno al eliminar cuenta',
            detalle: error?.message ??
                String(error),
        });
    }
});
app.post('/api/empleados/:idEmpleado/galeria', async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        const urlArchivo = String(req.body?.url ?? '').trim();
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
            return res.status(400).json({
                mensaje: 'ID de empleado inválido',
        ...
        const evidencias = obtenerFilas(resultadoInsercion);
        return res.status(201).json({
            mensaje: 'Imagen agregada a la galería',
            imagen: evidencias[0] ?? null,
        });
    }
    catch (error) {
...
app.get('/api/empleados/:idEmpleado/galeria', async (req, res) => {
    try {
        const idEmpleado = Number(req.params.idEmpleado);
        if (!Number.isInteger(idEmpleado) ||
            idEmpleado <= 0) {
        ...
        const filas = obtenerFilas(resultado);
        return res.status(200).json({
            galeria: filas.map((fila) => ({
                id_evidencia: Number(fila.id_evidencia),
                url_archivo: String(fila.url_archivo ?? ''),
                fecha_subida: fila.fecha_subida,
            })),
        });
    }
    catch (error) {
        console.error('Error consultando galería:', error);
        return res.status(500).json({
            mensaje: 'No se pudo consultar la galería',
            detalle: error?.message ??
                String(error),
        });
    }
...
console.log('NOTIFICACIONES CORREGIDAS: cliente y empleado separados');
