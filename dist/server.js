...
app.get('/api/empleados/disponibles', async (_req, res) => {
    ...
    `);
        const empleados = Array.isArray(respuesta?.recordset)
            ? respuesta.recordset
...
app.post('/api/reportes', async (req, res) => {
    try {
        const { tipoReporte, idReportante, tipoReportante, idServicio = null, idReportado = null, tipoReportado = null, categoria, descripcion, fotos = [], } = req.body;
        const idReportanteNumero = Number(idReportante);
        if (!Number.isInteger(idReportanteNumero) ||
            idReportanteNumero <= 0) {
            return res.status(400).json({
                mensaje: 'El usuario que realiza el reporte no es válido',
            });
        }
        if (tipoReporte !== 'aplicacion' &&
            tipoReporte !== 'usuario') {
            return res.status(400).json({
        ...
        if (tipoReporte === 'usuario') {
            ...
            if (!Number.isInteger(idReportadoNumero) ||
                idReportadoNumero <= 0) {
                return res.status(400).json({
            ...
            if (!Number.isInteger(idServicioNumero) ||
                idServicioNumero <= 0) {
                return res.status(400).json({
                    mensaje: 'El servicio relacionado con el reporte no es válido',
                });
            }
            if (idReportanteNumero === idReportadoNumero &&
        ...
        const idServicioFinal = tipoReporte === 'usuario'
            ? Number(idServicio)
            : null;
        const idReportadoFinal = tipoReporte === 'usuario'
            ? Number(idReportado)
            : null;
        const tipoReportadoFinal = tipoReporte === 'usuario'
            ? tipoReportado
            : null;
        const [resultado] = await database.execute(`
...
app.post("/api/login", async (req, res) => {
    ...
    `, [correoLimpio]);
            ...
            if (filas.length > 0) {
                ...
                const hashGuardado = String(empleado.password_hash ?? '').trim();
                console.log('ID encontrado:', empleado.id);
                console.log('Longitud del hash:', hashGuardado.length);
                console.log('Comienza como bcrypt:', hashGuardado.startsWith('$2'));
...
app.post('/api/servicios', async (req, res) => {
    try {
        ...
        const filasServicio = resultadoServicio.recordset ??
        ...
        */
        ...
        `, [idCategoria]);
        const empleados = resultadoEmpleados.recordset ??
...
app.get('/api/servicios', async (_req, res) => {
    try {
        ...
        ORDER BY
          ...
          s.hora_inicio DESC;
        `);
        const servicios = resultado?.recordset ??
...
// ==========================================
app.get('/api/notificaciones/empleado/:idEmpleado', async (req, res) => {
    try {
        ...
        const [resultado] = await database.execute(`
          ...
          `, [idEmpleado]);
        const notificaciones = resultado.recordset ??
            resultado.recordsets?.[0] ??
            [];
...
// ==========================================
app.patch('/api/notificaciones/:idNotificacion/leida', async (req, res) => {
    try {
        ...
        const [resultado] = await database.execute(`
          ...
          `, [idNotificacion]);
        const filasAfectadas = resultado.rowsAffected?.[0] ?? 0;
        if (filasAfectadas === 0) {
            return res.status(404).json({
...
app.post('/api/servicios/:idServicio/postular', async (req, res) => {
    try {
        ...
        const respuestaServicio = await database.query(`
          ...
          `);
        const servicios = Array.isArray(respuestaServicio?.recordset)
            ? respuestaServicio.recordset
            : Array.isArray(respuestaServicio?.recordsets?.[0])
        ...
        if (String(servicios[0].estado).trim() !==
            'Pendiente') {
            return res.status(400).json({
        ...
        const respuestaEmpleado = await database.query(`
          ...
          WHERE id_empleado = ${idEmpleado}
          `);
        const empleados = Array.isArray(respuestaEmpleado?.recordset)
            ? respuestaEmpleado.recordset
            : Array.isArray(respuestaEmpleado?.recordsets?.[0])
        ...
        const respuestaExistente = await database.query(`
          SELECT TOP 1
            id_postulacion,
            estado
          FROM postulaciones
          ...
          `);
        const postulacionesExistentes = Array.isArray(respuestaExistente?.recordset)
            ? respuestaExistente.recordset
            : Array.isArray(respuestaExistente?.recordsets?.[0])
                ? respuestaExistente.recordsets[0]
                : Array.isArray(respuestaExistente?.[0])
                    ? respuestaExistente[0]
                    : [];
        if (postulacionesExistentes.length > 0) {
            return res.status(409).json({
                mensaje: 'Ya te postulaste a este servicio',
                postulacion: postulacionesExistentes[0],
            });
        }
        const respuestaInsertar = await database.query(`
          ...
          `);
        const postulacionesInsertadas = Array.isArray(respuestaInsertar?.recordset)
            ? respuestaInsertar.recordset
            : Array.isArray(respuestaInsertar?.recordsets?.[0])
                ? respuestaInsertar.recordsets[0]
                : Array.isArray(respuestaInsertar?.[0])
                    ? respuestaInsertar[0]
                    : [];
        return res.status(201).json({
            mensaje: 'Postulación registrada correctamente',
            postulacion: postulacionesInsertadas[0] ?? {
...
app.get('/api/payment-methods/:fk_usuario', async (req, res) => {
    try {
        ...
        const [methods] = await database.query(`SELECT id_payment_method, fk_usuario, tipo, titular, numero_enmascarado, expiracion, fecha_creacion FROM payment_methods WHERE fk_usuario = ? ORDER BY fecha_creacion DESC`, [fk_usuario]);
...
app.get('/api/empleados/:idEmpleado/resumen-perfil', async (req, res) => {
    try {
        ...
        const respuestaResumen = await database.query(`
          SELECT
            e.id_empleado,
          ...
          GROUP BY
            e.id_empleado;
        `);
        const resumen = respuestaResumen?.recordset?.[0] ??
        ...
        const respuestaResenas = await database.query(`
          SELECT
            r.id_resena,
            r.id_reserva,
            r.calificacion_general,
          ...
          ORDER BY
            ...
            r.id_resena DESC;
        `);
        const resenas = Array.isArray(respuestaResenas?.recordset)
            ? respuestaResenas.recordset
            : Array.isArray(respuestaResenas
                ?.recordsets?.[0])
                ? respuestaResenas.recordsets[0]
                : Array.isArray(respuestaResenas?.[0])
                    ? respuestaResenas[0]
                    : Array.isArray(respuestaResenas?.rows)
